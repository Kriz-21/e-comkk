...
const Navbar = () => {
    ...
    // use chat gpt to {getcartCount() } not work line 81
        ...
        try {
            ...
            if (typeof getCartCount === 'function') {
                ...
                if (typeof count === 'number') {
                    setCartCount(count);
                } else if (count instanceof Promise) {
                    // If it's async, wait for the result
                    count.then(res => setCartCount(res ?? 0)).catch(() => setCartCount(0));
                }
            }
        } catch (err) {
            console.error('Error calling getCartCount:', err);
            setCartCount(0);
        }
    }, [getCartCount]);
